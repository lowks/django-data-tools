...
class QueryTest(TestCase):
    ...
    def test_simple(self):
        ...
        with self.assertNumQueries(4):
            last = 0
            for x, n in enumerate(RangeQuerySetWrapper(
                    User.objects.all(), step=1)):
    ...
    def test_with_callbacks(self):
        ...
        with self.assertNumQueries(4):
            for x, n in enumerate(RangeQuerySetWrapper(
                    User.objects.all(), step=1, callbacks=[
    ...
    def test_reverse_no_matches(self):
        self.assertEquals(len(list(RangeQuerySetWrapper(
            User.objects.filter(id=9000), step=-1))), 0)
